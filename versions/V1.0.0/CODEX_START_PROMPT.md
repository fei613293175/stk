...
完成后先由 `APK_BUILD_GATE` 生成唯一 Release APK Artifact，再由 `EMULATOR_ACCEPTANCE_GATE` 下载该 Artifact 执行 API 37 模拟器、全部新增按钮、截图比对、历史核心回归和首次安装。只下载通过验收的精确 Artifact，核对 SHA 与来源后复制到本机桌面 `商推客交付\1.0.0`，附完整计划、完成、测试和部署清单。未满足关闭清单不得进入下一版本。
