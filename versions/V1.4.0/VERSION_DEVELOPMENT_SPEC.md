...
- Android 构建、签名、PHP/Discuz、部署和交付只允许在线上服务器或 GitHub Actions 执行；包含模拟器的全量终验只允许在 GitHub Actions 执行。
...
当前 Commit 的 APK 先通过 `APK_BUILD_GATE` 生成唯一 Artifact；随后 `EMULATOR_ACCEPTANCE_GATE` 下载该 Artifact，通过合同、后端、Android、API 36 模拟器、本版全部新增交互、适用截图和升级安装验收后，才能从同一 Artifact 下载到本机。复制到 `%USERPROFILE%\Desktop\商推客交付\1.4.0\`，同时交付计划/完成/Owner测试/自动报告/视觉差异/部署/DNS/Build Info/Provenance/SHA/已知问题。
