...
@Composable
internal fun ApiRegisterScreen(api: StkApiClient, deviceId: String, onBack: () -> Unit, onAgreement: () -> Unit, onPrivacy: () -> Unit, onSuccess: (StkSession) -> Unit) {
    var phone by rememberSaveable { mutableStateOf("") }; var password by rememberSaveable { mutableStateOf("") }; var confirm by rememberSaveable { mutableStateOf("") }; var agreed by rememberSaveable { mutableStateOf(false) }; var pending by rememberSaveable { mutableStateOf(false) }; var busy by rememberSaveable { mutableStateOf(false) }; var error by rememberSaveable { mutableStateOf("") }
    Column(Modifier.fillMaxSize().padding(StkTokens.Space24), verticalArrangement = Arrangement.spacedBy(StkTokens.Space12)) {
        ...
        OutlinedTextField(password, { password = it }, Modifier.fillMaxWidth().testTag("register_password"), label = { Text("密码") })
        OutlinedTextField(confirm, { confirm = it }, Modifier.fillMaxWidth().testTag("register_password_confirm"), label = { Text("确认密码") })
...
@Composable
internal fun ApiResetScreen(api: StkApiClient, deviceId: String, onBack: () -> Unit) {
...
@Composable
internal fun ApiLegalScreen(api: StkApiClient, type: String, onBack: () -> Unit) {
...
@Composable
internal fun ApiHomeScreen(api: StkApiClient, session: StkSession, modifier: Modifier, onMe: () -> Unit, onDetail: (Int) -> Unit) {
    var projects by remember { mutableStateOf<List<ProjectItem>>(emptyList()) }; var loading by remember { mutableStateOf(true) }; var error by remember { mutableStateOf("") }; var reload by remember { mutableStateOf(0) }
...
@Composable
internal fun ApiMeScreen(api: StkApiClient, session: StkSession, deviceId: String, modifier: Modifier, onLoggedOut: () -> Unit) {
    var name by remember { mutableStateOf("") }; var uid by remember { mutableStateOf("") }; var mobile by remember { mutableStateOf("") }; var error by remember { mutableStateOf("") }; var busy by remember { mutableStateOf(false) }
    LaunchedEffect(session.accessToken) { api.get("/v1/me/summary", session.accessToken) { result -> result.onSuccess { raw -> runCatching { JSONObject(raw).getJSONObject("data") }.onSuccess { data -> name = data.optString("display_name"); uid = data.optInt("uid").toString(); mobile = data.optString("masked_mobile") }.onFailure { error = "资料响应无效" } }.onFailure { error = "资料加载失败" } } }
    Column(modifier.fillMaxSize().padding(StkTokens.Space16), verticalArrangement = Arrangement.spacedBy(StkTokens.Space16)) { Text("我的", style = MaterialTheme.typography.headlineSmall); if (error.isNotBlank()) Text(error, color = StkTokens.BrandAccent) else Card(Modifier.fillMaxWidth()) { Column(Modifier.padding(StkTokens.Space16)) { Text(name.ifBlank { "加载中" }); Text("UID：$uid", color = StkTokens.TextSecondary); Text("手机号：$mobile", color = StkTokens.TextSecondary) } }; OutlinedButton({ busy = true; val body = JSONObject().put("device_id", deviceId).put("refresh_token_family", session.refreshFamily); api.post("/v1/auth/logout", body.toString(), session.accessToken) { busy = false; onLoggedOut() } }, Modifier.fillMaxWidth().testTag("logout_confirm"), enabled = !busy) { Text(if (busy) "正在退出" else "退出登录") } }
}
