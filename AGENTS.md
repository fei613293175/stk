...
23. API 版本硬门禁（立即生效并适用于当前及后续每个小版本）：API 36（Android 16）正式稳定通道是唯一允许的 `compileSdk`、`targetSdk`、Android SDK platform、system image 和模拟器验收版本；API 37、其他非 API 36、Preview 和 Canary 均禁止出现在构建目标、依赖安装、GitHub Actions、模拟器、测试合同或交付证据中。版本目录必须逐版声明 `api_level: 36`、`api_channel: stable`、`forbidden_api_levels: all_except_36` 和 `explicitly_forbidden_api_levels: [37]`，禁止以升级、预览、canary 或临时 workaround 绕过该固定版本。
    工具包自身发行号（例如 `platform-tools 37.x` 或 `emulator 37.x`）不等于 Android API 37；只能在复用合同允许且不改变 API 36 platform/system image 的前提下使用。
