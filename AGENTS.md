...
23. API 版本硬门禁：API 36（Android 16）正式稳定通道是唯一允许的 `compileSdk`、`targetSdk`、Android SDK platform、system image 和模拟器验收版本；非 API 36（含预览/canary 通道及任何其他 API 版本）均禁止出现在构建目标、依赖安装、GitHub Actions、模拟器、测试合同或交付证据中。版本目录必须逐版声明 `api_channel: stable`、API 36 和 `forbidden_api_levels: all_except_36`，禁止以升级、预览、canary 或临时 workaround 绕过该固定版本。
