...
23. API 版本硬门禁（立即生效并适用于当前及后续每个小版本；详见 `docs/ANDROID_API_VERSION_POLICY.md`）：API 36（Android 16）正式稳定通道是唯一允许的 `compileSdk`、`targetSdk`、Android SDK platform、system image 和模拟器验收版本；API 37、其他非 API 36、Preview、Canary、Beta 和 RC 均禁止作为构建目标、依赖安装目标、GitHub Actions 镜像、模拟器目标或交付产物使用。版本目录必须逐版声明 `api_policy.allowed_api_level: 36`、`api_policy.api_channel: stable`、`api_policy.forbidden_api_levels: all_except_36`、`api_policy.explicitly_forbidden_api_levels: [37]` 和 `api_policy.on_mismatch: reject_before_build`；`[37]` 仅允许出现在规则的显式禁止清单中，禁止以升级、降级、预览、canary 或临时 workaround 绕过该固定版本。
    工具包自身发行号不等于 Android API level；只能使用稳定、非 Preview 工具，且不得安装或引用任何 API 37 platform、system image 或模拟器目标。
